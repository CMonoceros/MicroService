...
func TestMySQL(t *testing.T) {
	...
	c := &Config{
		...
		Active:       10,
		Idle:         5,
		IdleTimeout:  ctime.Duration(time.Minute),
		QueryTimeout: ctime.Duration(time.Minute),
		ExecTimeout:  ctime.Duration(time.Minute),
		TranTimeout:  ctime.Duration(time.Minute),
	}
	db := NewMySQL(c)
	defer db.Close()
	testPing(t, db)
